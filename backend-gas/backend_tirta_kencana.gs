...
function dispatchAction(fn, args) {
  var lock = null;
  if (WRITE_ACTIONS[fn]) {
...
function doPost(e) {
  try {
    ...
    var fn = body.fn;
    var args = body.args || [];
    var result = dispatchAction(fn, args); // [SECURITY FIX] sekarang lewat whitelist yang sama dengan doGet
...
function loginUser(name, password) {
  ...
  for (var i = 0; i < users.length; i++) {
    if (users[i].name === name) {
      if (users[i].passwordHash === hash) return { name: users[i].name, role: users[i].role };
